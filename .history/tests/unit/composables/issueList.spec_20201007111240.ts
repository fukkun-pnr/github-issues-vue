...
describe('composables/issues',() => {
    ...
    it('fetchIssueList', async () => {
        ...
        expect(setPagenationMock).toBeCalledWith(1);
        expect(issues.value).toEqual(result);

        await fetchIssueList(2);
        expect(getMock.mock.calls.length).toBe(2);
        expect(getMock.mock.calls[1][0]).toBe(githubUrl + "issues?page=2&amp;per_page=10");
        expect(setPagenationMock.mock.calls[1][0]).toBe(2);
    });
